...
### Do not retry (recall loop)

- Flying the 1-degree lattice for every kind the 2-degree pass never touched: 2-4x the Exact-spot solve for a handful of kinds (iteration 4).
- The measured weak-click bound in the map-wide sweep: +47% on the cold solve (iteration 1, first form).
- Widening verification's re-aim window for the remaining misses: the failed candidates sit 8-36 degrees from the real route (diagnosis before iteration 2).

### Loop closed 2026-09-07
...
### Remaining audit items, closed (2026-09-23, same day)

- **Watchtower waits 100 s** before killing a container it replaces (WATCHTOWER_TIMEOUT in npc_projects/scw_server, backed up first; the owner accepted that it applies to every container watchtower manages). Automatic deploys now drain an in-flight solve like a manual one.
- **Executes are cached.** Each smoke is an ordinary origin-scoped query and now shares the disk cache with /api/lineup and /api/execute/spots: an execute whose smokes are all cached answers at once, without a queue place; otherwise only the missing smokes are solved and written back. It now solves with the same spawn data the other two routes pass, so a shared cache entry is the answer any of them would have computed, and it counts as an interactive visitor so the cache warmer yields to it.
- **Votes are bounded.** One account can vote on at most 100 lineups at one spot (changing or withdrawing a vote is always allowed); a spot's read is aggregated in SQL and returns the 500 most-voted lineups plus the caller's own votes, instead of every row.
- **The Accuracy page says what it measures:** the physics given an exact release, not a person's aim, feet or jump timing; and that throws with eight or more bounces miss about one in six times against one in thirty or fewer for the rest (measured on the 18,353 re-graded throws). A first draft said "most misses" were eight-bounce throws; it is 10% of them, and that sentence never shipped.
- **Tests added:** ValidateCommand.MapArchiveMatch (the comparison split from its environment lookup so no test touches global state), and rig/test_warm_cache.py for the 429 and container-swap retries, run in CI; the swap test was confirmed to fail with its retry removed.
- **Lock files.** Directory.Build.props turns on packages.lock.json for every project with linux-x64 included; CI and the image build both restore with --locked-mode.
- **Image scan.** publish-image runs trivy on the booted image and stops the release on a critical or high issue that has a fix. The current image is clean.
- **Old images pruned weekly** (prune-images.yml, keeps the newest 60 versions; 259 existed, so the first run removes about 200).
- **Container hardening, still as root** by the owner's choice: every capability dropped but DAC_OVERRIDE (the prod data folder is partly owned by the host user), no-new-privileges, read-only root filesystem with a tmpfs /tmp. Verified on a user-owned copy of cs_italy before applying.
- Lineup queries reject an origin with more than three numbers, and the throw tie-break is one method (LineupSolver.ThrowOrder) used by both the sweep and the ranking.

Deliberately left: the two nearest-stand-spot searches (TargetSolver.NearestStandSpot and /api/standspot) answer different questions, one naming a spot from a 2D click and one deciding whether a 3D point can be stood on, so they stay separate; SmokeParams.UncalibratedDefault's 165u radius only pads the CLI's search box.

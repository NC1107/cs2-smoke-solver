...
4. **Sweep** the ±2u box hull (`GRENADE_DEFAULT_SIZE`) from `position` to `next`. If it hits a triangle:
   ...
   - **Angle damp (floor only)**: if impact speed `> DampGateSpeed (690 u/s)` *and* the impact angle is steeper than 60° (`|w·n|/|w| > 0.5`) *and* the surface is floor-like (`n.z > 0.7`), additionally scale by `(1.5 - |cos angle|)`. Walls never damp (validated: 0/122 gated wall bounces damped vs 68/76 gated ground bounces).
     The gate is judged on the velocity with the whole tick's gravity in it, even for a contact in the first half-step (MEASURED 2026-09-04: 590 steep bounces between 600 and 800 u/s separate cleanly at 688-690 on the full-tick speed and not on the half-step speed).
...
The one breakable prop the game bounced off, de_train's electronics enclosure doors at (-180, 6, -176), is now extracted as plain solid: a breakable prop counts as glass when its model prop_data says so (`base=Glass.Window`, which every passable pane on the ten probed maps carries; the enclosure is `Metal.break`, vertigo's rails `Wooden.Medium`), falling back to a "window"/"glass" model path where a community map's model has no base (cs_shelter).

...
The one breakable prop the game bounced off, de_train's electronics enclosure doors at (-180, 6, -176), is now extracted as plain solid: a breakable prop counts as glass only when its model path contains "window" or "glass", which every passable pane on the ten probed maps does.

...
Grenade collision uses a per-attribute-group filter derived from the physics interaction layers stored in the map (m_InteractAsStrings and m_InteractExcludeStrings), not from the ambiguous group names: player/NPC clips, sky volumes, and any group that excludes csgo_thrown_grenade do not block grenades (validated against 301 real bounce events and observed fly-throughs on de_dust2, and the 10,870-throw corpus replay of 2026-09-04), while Default/default, passbullets, and csgo_grenadeclip groups do.
